...
// they are usually defined externally in the tool code
    ...
    #define TOOL_DESCRIPTION_BREAK "ABC\nABC"
#endif
...
static const char *lblCopyrightText = "Copyright (c) 2024 Guilherme Oliveira (@0l1ve1r4)";
static const char *lblMoreInfoText = "More info:";
static const char *linkMailText = "gssantoz2012@gmail.com";
...
void GuiWindowAbout(GuiWindowAboutState *state)
{
    if (state->windowActive)
    {
        ...
        if (GuiLabelButton((Rectangle){ state->windowBounds.x + 90, state->windowBounds.y + 250, 165, 16 }, TextFormat("%s", TextToLower(TOOL_NAME)))) { OpenURL("https://discoverydatalab.ufop.br/"); }
        if (GuiLabelButton((Rectangle){ state->windowBounds.x + 90, state->windowBounds.y + 270, linkMailTextWidth, 16 }, linkMailText)) { OpenURL("mailto:gssantoz2012@gmail.com"); }
